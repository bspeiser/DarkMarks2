...
<%
	int max=999999;
	boolean odd=false;
	String lotdmax=request.getParameter("lotd");
	if (lotdmax!=null){
		...
		session.putValue(sLotdTitle,"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<A HREF='"+request.getRequestURI()+"?lotd=99999' style='font-size:smaller'>[all]</A>");
		%>
		...
		do	{
			...
			&nbsp;<FONT class="DATELINE"><%=lotdcur.getProperty("DATE_ADDED")%></FONT><SPAN class="DESCLINE"><BR/>&nbsp;&nbsp;&nbsp;
				<%=lotdcur.getProperty("DESCRIPTION")%>&nbsp;<A HREF="lotdcomm.jsp?lotd=<%=lotdcur.getProperty("MARK_ID")%>" class="COMLINK">(<%=lotdcur.getProperty("COMMENT_COUNT")%>)</A><BR/><IMG SRC="../images/clear.gif" height=5></SPAN></td></tr>
			<%						
